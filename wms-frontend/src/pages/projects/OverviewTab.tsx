...
export function OverviewTab({ projectId }: OverviewTabProps): React.JSX.Element {
  ...
  const avgProgress = wbs?.avg_progress ?? 0
  const completedNodes = wbs?.completed_nodes ?? 0
  const totalNodes = wbs?.total_nodes ?? 0
  const inProgressNodes = wbs?.in_progress_nodes ?? 0
  const totalItems = boq?.total_items ?? 0
  const overIssued = boq?.over_issued_count ?? 0
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-4 gap-4">
        <QuickStat
          ...
          label="Tiến độ WBS"
          value={`${avgProgress}%`}
          sub={`${completedNodes}/${totalNodes} hoàn thành`}
        />
        <QuickStat
          icon={<Package className="h-4 w-4 text-orange-500" />}
          label="BOQ"
          value={`${totalItems} hạng mục`}
          sub={overIssued > 0 ? `${overIssued} vượt định mức` : 'Trong kiểm soát'}
          accent={overIssued > 0 ? 'red' : undefined}
        />
        <QuickStat
          icon={<Users className="h-4 w-4 text-blue-500" />}
          label="Đội dự án"
          value={`${team_size ?? 0} người`}
        ...
        <QuickStat
          ...
          sub={`${totalNodes - completedNodes - inProgressNodes} chờ`}
        />
      </div>
