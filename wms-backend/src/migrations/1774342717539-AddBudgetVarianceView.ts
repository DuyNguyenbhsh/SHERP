...
export class AddBudgetVarianceView1774342717539 implements MigrationInterface {
  name = 'AddBudgetVarianceView1774342717539';

  public async up(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(`
      CREATE OR REPLACE VIEW v_monthly_project_budget_variance AS
      WITH budget_changes AS (
        -- Lấy tất cả thay đổi budget từ project_history
        SELECT
          ph.project_id,
      ...
      -- Giá trị đầu tháng (old_value của bản ghi sớm nhất) và cuối tháng (new_value của bản ghi muộn nhất)
      monthly_agg AS (
        SELECT
          bc.project_id,
          bc.reporting_year,
      ...
      -- Tạo chuỗi tháng liên tục cho mỗi dự án (fill gaps)
      ...
      -- Join chuỗi tháng với dữ liệu thay đổi thực tế
      filled AS (
        SELECT
          pmr.project_id,
          pmr.reporting_year,
          ...
          ma.first_old_value,
          ma.last_new_value,
          -- Carry forward: lấy new_value gần nhất từ các tháng trước
      ...
      SELECT
        ...
        f.reporting_year,
        f.reporting_month,
        -- Opening budget: giá trị đầu tháng, hoặc carry-forward từ tháng trước, hoặc budget hiện tại
        COALESCE(f.first_old_value, f.prev_closing, p.budget)  AS opening_budget,
        -- Closing budget: giá trị cuối tháng, hoặc giữ nguyên opening
        ...
        -- Variance % = (variance / opening) * 100, tránh chia cho 0
        CASE
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(
      `DROP VIEW IF EXISTS v_monthly_project_budget_variance;`,
    );
  }
}
